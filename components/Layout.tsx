...
import { 
  ...
  ChevronRight, PenTool, ClipboardList, BookHeart, BookOpen, Moon, Sun
} from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 flex font-sans">
      ...
      >
        ...
        <div className="h-24 flex items-center px-8 border-b border-white/5 bg-white/5">
          <div className="flex items-center gap-3">
             <div className="w-10 h-10 rounded-lg overflow-hidden shadow-lg shadow-indigo-500/30 flex-shrink-0">
                <img src="/Ministry-Manager/apple-touch-icon.png" alt="Ministry Manager" className="w-full h-full object-cover" />
             </div>
             <div>
        ...
        <div className="p-4 border-t border-white/5 bg-black/20">
          ...
          <div className="mt-3 px-2">
            <button
              onClick={toggleDark}
              className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-slate-400 hover:text-white hover:bg-white/10 transition-all"
            >
      ...
      {/* Main Content Area */}
      <div className="flex-1 flex flex-col min-w-0 h-screen relative bg-gray-50/50 dark:bg-slate-900">
        
        {/* Mobile Header */}
        <header className="lg:hidden h-16 bg-white/90 dark:bg-slate-800/90 backdrop-blur-md border-b border-gray-100 dark:border-slate-700 flex items-center px-4 justify-between z-40 sticky top-0 shadow-sm">
          <div className="flex items-center gap-2">
              <img src="/Ministry-Manager/apple-touch-icon.png" alt="Ministry Manager" className="w-7 h-7 rounded-md object-cover" />
              <span className="font-bold text-lg text-gray-800 dark:text-white">
                Ministry Mgr
              </span>
          </div>
          <div className="flex items-center gap-1">
              <button onClick={toggleDark} className="p-2 text-gray-500 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-full active:scale-95 transition-all" aria-label="Toggle dark mode">
                  {isDark ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
              </button>
              <button onClick={() => setSidebarOpen(true)} className="p-2 -mr-2 text-gray-600 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-full active:scale-95 transition-all">
        ...
        <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-800 border-t border-gray-200 dark:border-slate-700 z-40 pb-[env(safe-area-inset-bottom)] flex justify-around items-center h-[72px] shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
            {bottomTabs.map(item => {
               const active = isActiveLink(item.to);
               return (
                <Link 
                    key={item.to} 
                    to={item.to} 
                    className={`flex flex-col items-center justify-center w-full h-full space-y-1 active:scale-90 transition-transform relative ${active ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400 dark:text-slate-500'}`}
                >
            ...
            <button 
                ...
                className={`flex flex-col items-center justify-center w-full h-full space-y-1 active:scale-90 transition-transform ${sidebarOpen ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400 dark:text-slate-500'}`}
            >
                <Menu className="w-6 h-6" />
